...
class Database {
   ...
   public function execute_sp($sp_name = '', $sp_paramater = array()) {
      if (trim($sp_name)=='') {
      	   $result = array('ErrNum'=>'SYS001', 'ErrMsg'=>'Data tidak lengkap !');}
      else {
         $perintah = '('; 
         ...
         try {$this->conn = new PDO('mysql:host='.$this->host.';dbname='.$this->db_name,$this->db_user,$this->db_pass,$this->options);}
          	  ...
          	  $this->conn = null; return array ('ErrNum'=>'SYS001', 'ErrMsg'=>$Exception->getMessage());}  	
         try {   
            ...
            $result = array('ErrNum'=>'SYS002','ErrMsg'=>$exception->getMessage());}}
     	return $result;
   }
}   
?>
